...
# 1. Models: XGBoost, Bernoulli Naive Bayes, Logistic Regression
# IMPROVED: Added scale_pos_weight for XGBoost to handle class imbalance without distorting training data
# XGBoost is a gradient boosting model that often outperforms Random Forest on tabular data
...
results = []
for name, model in tqdm(models.items(), desc="Training Models", ncols=80):
    start = time.time()
...
# Set up selected baseline champion (XGBoost)
...
ablation_experiments = [
    {
        ...
        "Old_Value": 100,
        "New_Value": 200,
        "Params": {**xgb_baseline_params, "n_estimators": 200}
    },
    {
        "Experiment": "E2_max_depth_10",
        "Changed_Parameter": "max_depth",
        "Old_Value": 6,
        "New_Value": 10,
        "Params": {**xgb_baseline_params, "max_depth": 10}
    },
    {
        ...
        "Old_Value": 0.1,
        "New_Value": 0.05,
        "Params": {**xgb_baseline_params, "learning_rate": 0.05}
    },
    {
        ...
        "Old_Value": 0.8,
        "New_Value": 0.9,
        "Params": {**xgb_baseline_params, "subsample": 0.9}
    }
]
...
baseline_model = XGBClassifier(
    n_estimators=xgb_baseline_params["n_estimators"],
    max_depth=xgb_baseline_params["max_depth"],
    learning_rate=xgb_baseline_params["learning_rate"],
    subsample=xgb_baseline_params["subsample"],
    colsample_bytree=xgb_baseline_params["colsample_bytree"],
    scale_pos_weight=xgb_baseline_params["scale_pos_weight"],
    random_state=xgb_baseline_params["random_state"],
    n_jobs=xgb_baseline_params["n_jobs"]
)
...
for exp in tqdm(ablation_experiments, desc="Running Ablation Experiments", ncols=80):
    start = time.time()

    params = exp["Params"].copy()
    params_clean = {k: v for k, v in params.items() if k not in ["use_label_encoder", "eval_metric", "verbose"]}
    model = XGBClassifier(**params_clean)
...
print("\n" + "="*150)
print("PART C: ABLATION & TUNING - XGBoost Hyperparameter Experiments")
print("="*150)
print("\n=== XGBoost Ablation Log ===")
...
if best_experiment_name == "Baseline":
    final_model_params = xgb_baseline_params
else:
...
final_params = final_model_params.copy()
final_params_clean = {k: v for k, v in final_params.items() if k not in ["use_label_encoder", "eval_metric", "verbose"]}
final_model = XGBClassifier(**final_params_clean)
