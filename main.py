...
Original file is located at
    https://colab.research.google.com/drive/1i4VVYrBaUIbzLQkbvuRgEvgkYuR-ay09
...
3. Declare champion : Show that Random Forest wins the cross-validation race. That is why we evaluated and picked to use Random Forest.


Split :
YX - Data Prep & Pipeline using sklearn.pipeline.Pipeline object. Focus on implementing robust scaling, encoding and missing value handling.

Lionel - Model Selection, where we select 2-3 distinct algorithmic families (INCLUSIVE OF RANDOM FOREST OFC) then run them through K-fold cross validation.

Justin - Ablations and Tuning, where we take the winning champion model which will be Random Forest! and perform exactly 4 controlled experiments (tuning params) and detailing the results in an ablation log.

Ezra - Mechanical Failure Analysis where we extract 5 to 10 confident False Positives/Negatives from the validation data and analyze their specific feature values and propose technical fixes.

Jing Hai - Decision Making & Consolidation. To evaluate the business risks of loan defaults vs. rejected applications to justify shifting the 0.5 threshold.
```
"""
...
# Download latest version for dataset.


df = pl.read_csv("loan/loan.csv")
...
drop_cols = [
    ...
    "debt_settlement_flag", "hardship_flag", "pymnt_plan"
]
...
# Calculate dynamic categorical indices for SMOTENC after the imputer reorganizes columns
new_cat_indices = list(range(len(num_cols), len(num_cols) + len(cat_cols)))

# 6. Pipeline Step 2: Resampling
# Add an Undersampler to shrink the massive majority class first
# This reduces the "Fully Paid" class to a smaller number (e.g., 50,000 rows)
# Adjust this number based on how much RAM/Time you have.
undersampler = RandomUnderSampler(
    sampling_strategy={0: 50000}, # 0 is "Fully Paid"
    random_state=42
)

# Configure SMOTE-NC to balance the minority class up to the new majority size
# After undersampling, the minority ("Default"/1) will be synthetically boosted
# to match the 50,000 majority rows.
smote_nc = SMOTENC(
...
# This object is now ready to be handed off.
# Teammates can append their classifiers directly to this base pipeline.
...
#core_preprocessing_pipeline, X_train, X_test, y_train, and y_test are ready for handoff.
print("Data Prep and Pipeline Engineering is complete.")
'''
import polars as pl
import pandas as pd
import numpy as np
...
# 6. Pipeline Step 2: Resampling
# Adjusted to a 3:1 ratio to preserve real-world imbalance and improve precision
undersampler = RandomUnderSampler(
    sampling_strategy={0: 80000}, # Shrink "Fully Paid" to 90,000
    random_state=42
)
...
smote_nc = SMOTENC(
    categorical_features=new_cat_indices,
    random_state=42,
    sampling_strategy={1: 40000} # Boost "Default" to 30,000
)
...
# Show :
# 1. Extreme imbalance.
# 2. RandomUnderSampler chopped the massive 'Fully Paid' class down to 50,000.
# 3. SMOTE-NC synthetically cloned the 'Default' class up to 50,000, creating a perfectly balanced 50/50 dataset for the models to learn from.
...
# We manually push the data through the first two steps to get the intermediate numbers
...
X_train_smote, y_train_smote = smote_nc.fit_resample(X_train_under, y_train_under)
...
# Plot 2: After Undersampling (Reduced Fully Paid to 90k)
...
# Plot 3: After SMOTE-NC (Boosted Default to 30k, 3:1 Ratio)
...
axes[2].set_title(f"3. After SMOTE-NC (3:1 Ratio)\nTotal Rows: {len(y_train_smote):,}")
axes[2].bar_label(axes[2].containers[0], fmt='%d')
